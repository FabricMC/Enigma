...
public class GuiConfig implements Config {
    ...
    public enum LookAndFeel {
		...
		public void apply(GuiConfig config) {
	...
	private GuiConfig() {
		gson = new GsonBuilder()
			...
			.registerTypeAdapter(GuiConfig.class, (InstanceCreator<GuiConfig>) type -> this)
			.setPrettyPrinting()
			.create();
		try {
			this.loadConfig();
	...
	public void loadConfig() throws IOException {
		...
		if (configFile.exists()) {
			try {
				gson.fromJson(Files.asCharSource(configFile, Charset.defaultCharset()).read(), GuiConfig.class);
				loaded = true;
			} catch (Exception e) {
				e.printStackTrace();
			}
